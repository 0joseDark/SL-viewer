...
def realizar_login():
    # Usa os valores padrão definidos no início
    if login_second_life(USERNAME, PASSWORD, GRID_URI):
        messagebox.showinfo("Conexão", "Conexão estabelecida com sucesso!")
    else:
        messagebox.showerror("Conexão", "Falha ao se conectar ao Second Life. Verifique as credenciais e a conexão com a internet.")
